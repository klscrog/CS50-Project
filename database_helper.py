...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect(DATABASE, timeout=30)  # Increase timeout to 30 seconds
...
def get_pool(pool_id):
    ...
    if pool:
        ...
        pool['created_at'] = datetime.strptime(pool['created_at'], '%Y-%m-%d %H:%M:%S')  # Adjust format as needed
    return pool
